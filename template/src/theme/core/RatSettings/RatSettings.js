...
import { ThemeLayoutConfigs } from '@app/layouts';
...
const RatSettings = props => {
    const dispatch = useDispatch();
    const user = useSelector(({ auth }) => auth.userAuth);
    const themes = useSelector(({ theme }) => theme.settings.themes);
    ...
    const handleUpdate = useDebounce(newSettings => {
        if (user.role === 'guest') {
            ...
            dispatch(userAuthActions.updateUserSettings(newSettings));
        }
    }, 300);
...
export default memo(RatSettings);
