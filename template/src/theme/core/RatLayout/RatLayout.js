...
import { ThemeLayouts } from '@app/layouts';
import { generateSettings, setSettings } from '@store/modules/theme/settings/settingsSlice';
...
function RatLayout(props) {
    const dispatch = useDispatch();
    const settings = useSelector(({ theme }) => theme.settings.current);
    const defaultSettings = useSelector(({ theme }) => theme.settings.defaults);
    ...
    // console.warn('::RatLayout:: rendered');

    const Layout = useMemo(() => ThemeLayouts[settings.layout.style], [settings.layout.style]);

    return _.isEqual(newSettings.current, settings) ? <Layout classes={{ root: classes.root }} {...props} /> : null;
}

export default memo(RatLayout);
