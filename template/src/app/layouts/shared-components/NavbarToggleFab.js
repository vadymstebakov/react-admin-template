...
function NavbarToggleFab(props) {
    ...
    const config = useSelector(store => store.theme.settings.current.layout.config);
    ...
    return (
        <Tooltip title="Show Navigation" placement={config.navbar.position === 'left' ? 'right' : 'left'}>
            <Fab
                ...
                onClick={ev => dispatch(mdDown ? navbarActions.navbarToggleMobile() : navbarActions.navbarToggle())}
                disableRipple
            >
